...
void main() {
  group('ValueNotifier Integration Tests', () {
    test('should notifier can add listener', () {
    ...
    test('JoltValueListenable cannot be set value', () {
      final signal = Signal(0);
      final notifier = signal.notifier;
      final listenable = notifier.toListenableSignal();

      expect(() {
        (listenable as dynamic).value = 1;
      }, throwsA(isA<UnimplementedError>()));

      expect(() {
        (listenable as dynamic).set(1);
      }, throwsA(isA<UnimplementedError>()));
    });
  });
}
        ...
        body: Column(
          children: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                // ReadonlySignal doesn't have a value setter
                // This is expected behavior to prove it's read-only
                try {
                  // Intentional: ReadonlySignal has no value setter
                } catch (e) {
                  // Expected to fail
                }
              },
