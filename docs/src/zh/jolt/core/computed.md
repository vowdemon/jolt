...
使用 `.value` 属性、`.get()` 方法或直接调用 computed 对象来读取值，**这会创建响应式依赖并触发计算**。如果依赖已经改变，会重新计算；否则返回缓存的值。
...
Effect(() {
  ...
  print(doubled.get()); // 使用 .get()
  print(doubled()); // 直接调用
});

count.value = 5; // 所有方式都会触发重新计算
```
...
3. **依赖追踪**：在 getter 函数中使用 `.value`、`.get()` 或 `call()` 访问其他响应式值会建立依赖关系。使用 `.peek` 不会建立依赖。
