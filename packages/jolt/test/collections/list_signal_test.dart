...
void main() {
  group("ListSignal", () {
    ...
    test("delegates List read operations to the current value", () {
      final baseline = [1, 2, 3, 4, 5];
      final signal = ListSignal<int>(List<int>.from(baseline));

      expect(signal.length, baseline.length);
      ...
      expect(signal.firstWhere((value) => value > 3),
          baseline.firstWhere((value) => value > 3));
      expect(signal.indexOf(3), baseline.indexOf(3));
      expect(signal.indexOf(3, 2), baseline.indexOf(3, 2));
      expect(signal.indexWhere((value) => value == 4),
          baseline.indexWhere((value) => value == 4));
      expect(signal.lastIndexOf(3), baseline.lastIndexOf(3));
      expect(signal.lastIndexWhere((value) => value == 3),
          baseline.lastIndexWhere((value) => value == 3));
      expect(signal.lastWhere((value) => value < 3),
          baseline.lastWhere((value) => value < 3));
      expect(
      ...
      expect(signal.reversed.toList(), baseline.reversed.toList());
      expect(signal.sublist(1, 3), baseline.sublist(1, 3));
      expect(signal.elementAt(2), baseline.elementAt(2));

      final visited = <int>[];
      signal.forEach(visited.add);
      expect(visited, baseline);
      ...
      expect(signal.skip(2).toList(), baseline.skip(2).toList());
      expect(signal.skipWhile((value) => value < 3).toList(),
          baseline.skipWhile((value) => value < 3).toList());
      ...
      expect(signal.isEmpty, baseline.isEmpty);
      expect(signal.isNotEmpty, baseline.isNotEmpty);
      expect(signal + [6], baseline + [6]);
    });

    test("delegates single-element queries", () {
      final signal = ListSignal<int>([7]);

      expect(signal.single, 7);
      expect(signal.singleWhere((value) => value == 7), 7);
    });

    test("read queries establish dependencies on current list state", () {
      final signal = ListSignal<int>([1, 2, 3]);
      final values = <int>[];
