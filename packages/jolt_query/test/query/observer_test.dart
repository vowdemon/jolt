...
void main() {
  group('QueryObserver lifecycle', () {
    ...
    test(
      ...
      var fetchCalls = 0;
      final target = query(
        key: QueryKey(<Object?>['double-dispose-polling']),
        fetch: (context) => ++fetchCalls,
        retry: RetryPolicy.none,
    ...
    test('watchQuery switches entries and carries previous view to placeholder',
      ...
      final observer = harness.client.watchQuery(() {
        ...
        return query(
          key: QueryKey(<Object?>['watched', id]),
          fetch: (context) => id,
          retry: RetryPolicy.none,
        )
            .select((value) {
    ...
    test('watchQuery key changes fetch once and reuse a fresh cached key',
      ...
      final observer = harness.client.watchQuery(() {
        ...
        return query(
          ...
          retry: RetryPolicy.none,
        ).placeholder((previous) => previous).observer(
    ...
    test(
      ...
      Query<int> recipeFor(int id) => query(
            key: QueryKey(<Object?>['optional-key-change', id]),
            fetch: (_) {
    ...
    test(
      ...
      final observer = harness.client.watchQuery(() {
        ...
        return query<int>(
          key: QueryKey(<Object?>['same-key-retarget']),
          fetch: (_) {
    ...
    test('same-key disabled-to-enabled transition activates a stale query',
      ...
      var calls = 0;
      final raw = query(
        key: QueryKey(<Object?>['same-key-enable']),
        fetch: (_) => ++calls,
        retry: RetryPolicy.none,
      );
    ...
    test('same-key plan updates preserve an unchanged polling schedule',
      ...
      harness.client.watchQuery(() {
        final currentRevision = revision.value;
        return query(
          key: QueryKey(<Object?>['same-key-polling']),
          fetch: (_) {
    ...
    for (final removal in <String>['removeQueries', 'queryCache.clear']) {
      test('$removal can race a same-key retarget without losing observation',
        ...
        final observer = harness.client.watchQuery(() {
          final currentRevision = revision.value;
          return query<int>(
            key: QueryKey(<Object?>['same-key-removal-race']),
            fetch: (_) {
              if (currentRevision == 1) secondPlanCalls += 1;
              return currentRevision + 10;
            },
            retry: RetryPolicy.none,
          ).select((value) => '$currentRevision:$value').observer(
                enabled: false,
              );
        });
        await harness.pump();
  ...
  group('initial and placeholder data', () {
    test(
      ...
      var resolverCalls = 0;
      final raw = query<int>(
        key: QueryKey(<Object?>['absent-placeholder']),
        fetch: (context) => 1,
        retry: RetryPolicy.none,
      );
      final target = raw.placeholder((previous) {
        resolverCalls += 1;
    ...
    test('raw present-null initial data is shared before selection', () {
      ...
      final raw = query<String?>(
        key: QueryKey(<Object?>['nullable-initial']),
        fetch: (context) {
          fetchCalls += 1;
          return 'network';
        },
      ...
      final target = raw
          .initialData(null, updatedAt: updatedAt)
    ...
    test('first initial writer wins for one structural key', () {
      ...
      addTearDown(harness.dispose);
      final first = query(
        key: QueryKey(<Object?>['first-writer']),
        fetch: (context) => 'network',
        retry: RetryPolicy.none,
      );
      final second = query(
        key: QueryKey(<Object?>['first-writer']),
      ...
      harness.client.observeQuery(
        first.initialData('first').observer(enabled: false),
      );
      harness.client.observeQuery(
        second.initialData('second').observer(enabled: false),
      );
    ...
    test('final present-null placeholder bypasses selector and shared cache',
      ...
      var selectorCalls = 0;
      final raw = query<int?>(
        key: QueryKey(<Object?>['nullable-placeholder']),
        fetch: (context) => 1,
        retry: RetryPolicy.none,
      );
      final target = raw
  ...
  group('selection and fine-grained tracking', () {
    test('selector failure stays local to its observer', () async {
      ...
      addTearDown(harness.dispose);
      final raw = query(
        key: QueryKey(<Object?>['selector-failure']),
        fetch: (context) => 1,
        retry: RetryPolicy.none,
      );
      ...
      final healthy = harness.client.observeQuery(
        raw.select((value) => value * 10).observer(enabled: false),
      );
    ...
    test('selection memo and equality preserve data-only readers', () async {
      ...
      final raw = query(
        key: QueryKey(<Object?>['selection-memo']),
        fetch: (context) => firstRaw,
        retry: RetryPolicy.none,
      );
    ...
    test('fetch-only transitions notify fetch and whole but not data',
      ...
      final completion = Completer<int>();
      final raw = query(
        key: QueryKey(<Object?>['field-tracking']),
        fetch: (context) => completion.future,
        retry: RetryPolicy.none,
      );
      harness.client.setQueryData(raw, 1);
      final observer = harness.client.observeQuery(
    ...
    test('unchanged refreshes preserve the complete result identity', () {
      ...
      addTearDown(harness.dispose);
      final raw = query(
        key: QueryKey(<Object?>['stable-result-identity']),
        fetch: (_) => 1,
        retry: RetryPolicy.none,
      );
      harness.client.setQueryData(raw, 1);
    ...
    test('manual data writes participate in fetched-after-mount state',
      ...
      final afterMount = query(
        key: QueryKey(<Object?>['manual-after-mount']),
        fetch: (_) => 1,
        retry: RetryPolicy.none,
      );
      final emptyObserver = harness.client.observeQuery(
        afterMount.observer(enabled: false),
      );
  ...
  group('activation and environment policies', () {
    ...
    test('retryOnMount controls remount after an initial failure', () async {
      ...
      var calls = 0;
      final raw = query<int>(
        key: QueryKey(<Object?>['retry-on-mount']),
        fetch: (context) {
          calls += 1;
      ...
      final disabledRetry = harness.client.observeQuery(
        raw.observer(retryOnMount: false),
      );
      await harness.pump();
      ...
      harness.client.observeQuery(raw.observer(retryOnMount: true));
      await harness.pump();
      expect(calls, 2);
    });
    ...
    test(
      ...
      var calls = 0;
      final target = query(
        key: QueryKey(<Object?>['duration-fresh-environment']),
        fetch: (context) => ++calls,
        retry: RetryPolicy.none,
      )
          .initialData(
    ...
    test('disabled resolver remains fresh across environment transitions',
      ...
      var calls = 0;
      final target = query(
        key: QueryKey(<Object?>['resolver-environment-events']),
        fetch: (_) => ++calls,
        retry: RetryPolicy.none,
      ).initialData(1).observer(
            enabled: false,
    ...
    test('duration expiry publishes stale state without fetching', () async {
      ...
      var calls = 0;
      final target = query(
        key: QueryKey(<Object?>['duration-expiry']),
        fetch: (context) => ++calls,
        retry: RetryPolicy.none,
      ).initialData(1, updatedAt: harness.clock.wallNow()).observer(
    ...
    test('disabled observers never make aggregate cache state stale', () async {
      ...
      addTearDown(harness.dispose);
      final raw = query(
        key: QueryKey(<Object?>['aggregate-freshness']),
        fetch: (_) => 2,
        retry: RetryPolicy.none,
      );
      harness.client.setQueryData(raw, 1);
    ...
    test('focus regain refetches after duration data becomes stale', () async {
      ...
      var calls = 0;
      final target = query(
        key: QueryKey(<Object?>['duration-focus-refetch']),
        fetch: (context) => 1 + ++calls,
        retry: RetryPolicy.none,
      ).initialData(1, updatedAt: harness.clock.wallNow()).observer(
            staleTime: StalePolicy.duration(const Duration(seconds: 5)),
            refetchOnMount: RefetchPolicy.never,
    ...
    test('freshness, focus, and immutable overrides control auto fetch',
      ...
      var freshCalls = 0;
      final fresh = query(
        key: QueryKey(<Object?>['fresh']),
        fetch: (context) => ++freshCalls,
        retry: RetryPolicy.none,
      ).initialData(1, updatedAt: harness.clock.wallNow()).observer(
            staleTime: StalePolicy.duration(const Duration(hours: 1)),
          );
      ...
      var focusCalls = 0;
      final focus = query(
        key: QueryKey(<Object?>['focus']),
        fetch: (context) => ++focusCalls,
        retry: RetryPolicy.none,
      ).initialData(1).observer(
            refetchOnMount: RefetchPolicy.never,
      ...
      final immutable = query(
        key: QueryKey(<Object?>['immutable']),
        fetch: (context) => ++immutableCalls,
        retry: RetryPolicy.none,
      ).initialData(1).observer(
            staleTime: StalePolicy.immutable,
            refetchOnMount: RefetchPolicy.always,
    ...
    test('online pause continues without a duplicate reconnect fetch',
      ...
      var calls = 0;
      final target = query(
        key: QueryKey(<Object?>['online-gate']),
        fetch: (context) => ++calls,
        retry: RetryPolicy.none,
    ...
    test('offline-first performs its initial attempt while offline', () async {
      ...
      final observer = harness.client.observeQuery(
        query(
          key: QueryKey(<Object?>['offline-first']),
          fetch: (context) => ++calls,
          retry: RetryPolicy.none,
    ...
    test('always mode suppresses default reconnect but allows an override',
      ...
      final defaultTarget = query(
        key: QueryKey(<Object?>['always-default']),
        fetch: (context) => ++defaultCalls,
        retry: RetryPolicy.none,
        networkMode: NetworkMode.always,
      ).initialData(1).observer(refetchOnMount: RefetchPolicy.never);
      harness.client.observeQuery(defaultTarget);

      var overrideCalls = 0;
      final overrideTarget = query(
        key: QueryKey(<Object?>['always-override']),
        fetch: (context) => ++overrideCalls,
        retry: RetryPolicy.none,
        networkMode: NetworkMode.always,
      ).initialData(1).observer(
            refetchOnMount: RefetchPolicy.never,
  ...
  group('polling', () {
    test('multiple pollers deduplicate and never overlap entry work', () async {
      ...
      final fiveSecondTarget = raw.initialData(0).observer(
            refetchOnMount: RefetchPolicy.never,
            pollingInterval: const Duration(seconds: 5),
          );
      final sevenSecondTarget = raw.initialData(0).observer(
            refetchOnMount: RefetchPolicy.never,
    ...
    test('reactive polling intervals replace timers and honor focus', () async {
      ...
      var calls = 0;
      final raw = query(
        key: QueryKey(<Object?>['dynamic-poll']),
        fetch: (context) => ++calls,
        retry: RetryPolicy.none,
      );
      harness.client.watchQuery(
